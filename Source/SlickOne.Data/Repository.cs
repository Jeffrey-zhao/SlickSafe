...
namespace SlickOne.Data
{
    ...
    public class Repository : IRepository
    {
        ...
        public IList<T> GetByName<T>(string colName, string value) where T : class
        {
            ...
            var sql = string.Format("SELECT * FROM {0} WHERE LOWER({1})=@colValue", tblName, colName);

            try
            {
                using (IDbConnection conn = SessionFactory.CreateConnection())
                {
                    IList<T> dataList = SqlMapper.Query<T>(conn, sql, new { colValue = value.ToLower() }).ToList();
                    return dataList;
                }
            }
            catch(Exception ex)
